export class EntregaController{
    ...
    AtribuirMotorista = async(req, res, next) => {
        try{
            const {id} = req.params;
            const {idMotorista} = req.body;

            if (!idMotorista){
                return res.status(400).json({erro: "O campo de IdMotorista é obrigatório"})
            }
            const entregaAtualizada = await this.entregaServices.atribuirEntrega(id, idMotorista);
